...
| 10 | ErrorBoundary / fallback | ✅ Выполнено | Создан `src/components/ErrorBoundary.tsx` — React-класс с `getDerivedStateFromError` + `componentDidCatch` (логирует в `console.error`, без новой зависимости). `App.tsx` оборачивает `GlobeScene` в `<ErrorBoundary fallback={<GlobeSceneFallback />}>` — fallback показывает неоновую панель «3D-глобус недоступен … 2D-карта справа продолжает работать». 2D-карта (SVG, без WebGL) продолжает работать независимо. Добавлен `src/components/ErrorBoundary.test.tsx` (5 тестов): рендерит children без ошибки, рендерит fallback при ошибке, логирует ошибку, не пропускает ошибку наружу, не ломает соседние сабтри. `AGENTS.md` обновлён (описан компонент + тест добавлен в список покрытых). 610 тестов проходят, lint чист, build успешен |
...
2. ~~**Затем работа 2** (ErrorBoundary)~~ — ✅ выполнено. `ErrorBoundary.tsx` создан, `App.tsx` оборачивает `GlobeScene`, fallback-панель «3D-глобус недоступен», `ErrorBoundary.test.tsx` (5 тестов), `AGENTS.md` обновлён. 610 тестов зелёные, lint чист, build успешен.
