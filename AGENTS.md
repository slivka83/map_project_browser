...
- `src/utils/threeHelpers.ts` holds `quatFromNormal` (the quaternion that rotates +Z onto a normal), shared by `AuxSurface` and `TangencyRings` (was duplicated inline). `AuxSurface` and `TangencyRings` now memoize their geometry (`computeAuxSurfaceParams` / `computeTangencyRing`) to match the `Rays` precedent and the "wrap D3/ray math in `useMemo`" hard rule.
