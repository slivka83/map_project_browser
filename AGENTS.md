...
- 2D `src/components/Map2D.tsx`: responsive SVG (measures its container via `ResizeObserver`) whose projection is fitted with `fitProjectionToView` so the globe always fills the viewport with a small uniform margin, regardless of the chosen projection; graticule, coastlines from `land50GeoJson` (≥50m detail), optional country-border layer from `countriesGeoJson`, and optional Tissot indicatrices via `d3Geo.geoCircle().radius(5)` on a 30° grid. The **Индикатрисы Тиссо**, **Детализация карты** and **Границы стран** buttons are always visible together as a single horizontal row in the map's top-right corner. When detail is off, `Map2D` draws the lightweight 110m `geoJsonData` land with 110m borders (`countries110GeoJson`) instead of the 50m datasets. The map's **bottom-right corner** shows a small glass label `Искажение площади: NN%` — the area-weighted area distortion of the current projection settings, computed by `computeAreaDistortion` (in `projectionMapper.ts`): it samples local area-scale factors on a fine lon/lat grid, takes the projection centre `(lambda0, phiOrigin)` — by construction the least-distorted point — as the 100% reference, and reports the mean relative area excess (equal-area projections → ~0%; scale-invariant to `scaleFactor`). Cells that straddle the antimeridian, fold onto themselves, or are stretched far past the centre scale (projection discontinuities) are skipped so they can't masquerade as distortion.
- 3D scene `src/components/GlobeScene.tsx` composes five thin components (all flat under `components/`, no `Scene3D/` subfolder): `Globe.tsx` (transparent dark sphere with neon-blue **3D-line** coastlines + graticule — NOT the `CanvasTexture`/orthographic variant from spec §6), `AuxSurface.tsx` (fully transparent neon-orange **wireframe** of the developable surface — its own meridians + parallels; cylinder for cylindrical, cone for conic, plane for azimuthal; the azimuthal plane is the projection plane tangent to the globe), `IntersectionDisks.tsx` (hollow white neon rings marking the **actual** intersection of the aux surface with the globe — 0, 1 or 2 circles; a cylinder of radius R·scaleFactor yields two circles when immersed, one when tangent, none when it encloses the sphere; the component renders nothing when the surface does not touch the globe), `LightSource.tsx` (yellow glowing core at the globe centre — the projection's light source), and `Rays.tsx` (yellow light-beam fan from the globe centre to the aux surface). The ray fan is **analytic** (computed from the projection), starts at the globe centre, and renders with `depthTest={false}` so the portion inside the globe stays visible; it renders independently of whether the geo dataset has loaded. `GlobeScene` adds `OrbitControls` (from `@react-three/drei`, `makeDefault`, pan disabled) so the user can rotate/zoom the camera and see the aux surface's true 3D orientation — without it, a tangent plane on the visible hemisphere always appears to face the viewer. The azimuthal aux-plane wireframe rotates its grid points directly by the tangent-basis quaternion (not via the group `quaternion` prop) so its orientation is always correct.
- `src/utils/auxSurfaceGeometry.ts` is the **single source of truth** for all 3D geometry: `computeAuxSurfaceParams`, `computeAuxGraticule`, `computeTangencyRing`, `computeAuxSphereIntersections` (the real aux-surface↔globe intersection circles), `computeCentralMeridianRays`, `computeTangentBasis`, `lonLatToVec3` (`RADIUS = 10`). Surface, ring, and rays derive from the same math so they can never drift apart. (Replaces the old `rayGeometry.ts`.) `computeCone(phi1, phi2, radius, scaleFactor)` supports both **tangent** (φ1 = φ2) and **secant** (φ1 ≠ φ2, same hemisphere) cones; `gamma` tilts the cylinder/cone/plane for oblique & transverse aspects; `computeCentralMeridianRays` honours the azimuthal `azLight` modes (`center` → gnomonic, `antipode` → stereographic, `infinity` → parallel orthographic beams, `math` → centre beams rendered as dashed formula vectors).
