...
- `src/utils/projectionMapper.ts` maps (`family` × `distortion`) → a D3 projection, then applies `proj.rotate([-lambda0, -phiOrigin]).scale(100 * scaleFactor).translate([400 + falseEasting, 300 + falseNorthing])`. Note the **negative** signs. Conic projections use a single tangent parallel `parallels([phiOrigin, phiOrigin])` (the store dropped `phi1`/`phi2`). Conic projections are degenerate near the equator, so the parallel falls back to `30°` when `|phiOrigin| < 10` (matching the aux-surface geometry). `fitProjectionToView` overrides scale/translate to fit the 2D map to its container (the fixed `scale(100)` is only used by the 3D ray math).
- 2D `src/components/Map2D.tsx`: responsive SVG (measures its container via `ResizeObserver`) whose projection is fitted with `fitProjectionToView` so the globe always fills the viewport with a small uniform margin, regardless of the chosen projection; graticule, coastlines from `geoJsonData`, and optional Tissot indicatrices via `d3Geo.geoCircle().radius(5)` on a 30° grid.
