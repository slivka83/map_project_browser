...
рога» при наклоне). Для цилиндрических семейств размер карты подгоняется **по окну** (полоса
±`CLIP_LAT` вокруг центральной параллели, ограниченная краями трубы и полюсами): масштаб и
перевод считаются аналитически из сырых строк окна, чтобы выбранная параллель легла ровно в
середину, а `clipExtent` ставится на те же строки. Окно никогда не включает зажатую «шапку»
(у Меркатора её высота на 89.5° ≈ 5.4 против 3.1 у края окна), поэтому она не может ни
схлопнуть масштаб, ни размазаться по карте. Сам закон высоты зажимается лишь до ±`CLAMP_LAT`
(чуть дальше края окна) — только чтобы у Меркатора не уйти в бесконечность на полюсе. Для
цилиндрических проекций всегда включено `precision(0)` (без адаптивной передискретизации),
иначе d3 изгибает прямые параллели в дуги и карта выглядит «яйцом». Конические и азимутальные
