...
но `rotZ = 0` для **цилиндрического**. 2D-карта цилиндрических проекций — это **плоская
стандартная карта**: в поворот входит только центральный меридиан
(`rotate([-(lambda0), 0, 0])`), а центральная широта φ₀ (Параллель 1) карту **не
наклоняет** — при наклоне 3D-трубы карта **только смещается**: окно ±`CLIP_LAT` вокруг
выбранной параллели сдвигается так, что эта параллель оказывается в средней строке.
Наклон 3D-трубы (её ось отклоняется от оси Земли ровно на φ₀) виден в 3D-сцене и в
индикаторе оси; лучи **привязаны к трубе** и поворачиваются вместе с ней (через
`auxPointToWorld`), а строились от проекции с φ₁ = 0. `lambda0` / `phiOrigin`
...
cos φ_s, равнопромежуточная → y ∝ φ. Карта построена по **двухслойной модели**: цилиндр и его
градуировка (сетка координат с полюсами у краёв окна) **неподвижны** — сетка рисуется из
полностью статичной проекции без поворотов (`lambda0 = phiOrigin = 0`); поверх неё скользит
**слой материков** (вместе с границами стран): Долгота вращает его вокруг земной оси
(`rotate([-lambda0, 0, 0])`), Параллель сдвигает вертикально жёстким смещением трансляции,
ровно так, что выбранная параллель ложится на среднюю строку окна (величина сдвига берётся
точным зондом закона высоты, без линейной аппроксимации). Вертикальная координата
**периодична**: широта заворачивается в полосу ±`CLIP_LAT` с периодом 2·`CLIP_LAT` — при
прокрутке за полюс с противоположной стороны входит противоположный (южный полюс появляется
сверху, северный снизу), лента бесконечна в обе стороны. Материки и границы заранее
обрезаются по ±`CLIP_LAT` (`geoBandClip.cutFeatureCollectionToBand`), поэтому ничего не
пересекает шов закольцовки; Map2D рисует по три экземпляра слоя (`createProjectionTiles`),
составленных встык через высоту полосы. Оба слоя делят один аналитический
фит по полосе ±`CLIP_LAT` (размер окна постоянен при любом положении ползунков), и каждый
...
полюсе. Всегда включено `precision(0)`: слои никогда не наклоняются, параллели остаются
прямыми горизонтальными линиями при любом положении ползунков.
