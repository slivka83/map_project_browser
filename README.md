...
(третий компонент `gamma` — наклон/третье вращение). Он зашивается во **все** семейства,
включая цилиндрическое: наклон цилиндра меняет, какой кусок глобуса ложится на трубу, поэтому
карта **перерисовывается** при наклоне — материки и сетка смещаются/искажаются, но сама рамка
карты остаётся прямоугольником с прямыми параллелями (без «яйца»), потому что проекция
строится с `precision(0)`. `makeCylindricalProjection(distortion, scaleFactor)` сохраняет
выбранное свойство своим законом высоты со стандартной параллелью на контакте ±φ_s
...
Конические и азимутальные проекции также зашивают `gamma`. Для конических проекций берётся
пара параллелей `parallels([φ1, φ2])`
