...
Вращение использует **отрицательные** знаки: `.rotate([-lambda0, -phiOrigin, -gamma])`
(третий компонент `gamma` — наклон/третье вращение, даёт поперечные/косые аспекты) и
зашивается во **все** семейства, включая цилиндрическое. Цилиндрическая карта — это
**геометрическое развёртывание 3D-цилиндра**, вычисленное в ЛОКАЛЬНОЙ системе координат
цилиндра: поворот `[-lambda0, -phiOrigin, -gamma]` переводит глобус в эту систему, поэтому
наклон учитывается ДО самой цилиндрической проекции — без сингулярности поперечной
Меркатора. `makeCylindricalProjection(distortion, scaleFactor)` сохраняет выбранное свойство
своим законом высоты со стандартной параллелью на контакте ±φ_s (φ_s = arccos(`scaleFactor`)):
равноугольная → секущая Меркатора, равновеликая → y ∝ sin φ / cos φ_s, равнопромежуточная →
y ∝ φ. Проекция **обрезается по конечной трубе** (±`CLIP_LAT` от оси цилиндра), поэтому под
наклоном нет ухода в бесконечность у полюса — точки за пределами трубы просто не лежат на
поверхности, как и в 3D. Конические и азимутальные проекции также зашивают `gamma`. Для
конических проекций берётся пара параллелей `parallels([φ1, φ2])`
