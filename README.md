...
cos φ_s, равнопромежуточная → y ∝ φ. Труба **конечна**: окно карты не выходит за ±`CLIP_LAT`
(поэтому содержимое за пределами трубы не размазывается по карте — не возникает «бараньего
рога» при наклоне). Для цилиндрических семейств масштаб карты считается **по всей полосе**
±`CLIP_LAT` и **не зависит от Параллели 1**: её движение — это чистая вертикальная прокрутка
карты (симметрично горизонтальной прокрутке по долготе), без изменения масштаба. Перевод
центрирует выбранную параллель в средней строке, а `clipExtent` ставится на краях полосы:
зажатая полярная «шапка» Меркатора (её высота на 89.5° ≈ 5.4 против 3.1 у края) не попадает
на сдвинутую карту — за краями полосы остаётся пустой фон. Сам закон высоты зажимается лишь
до ±`CLAMP_LAT` (чуть дальше края окна) — только чтобы у Меркатора не уйти в бесконечность
на полюсе. Для цилиндрических проекций всегда включено `precision(0)` (без адаптивной
передискретизации), иначе d3 изгибает прямые параллели в дуги и карта выглядит «яйцом».
