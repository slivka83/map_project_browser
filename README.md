...
Поддерживаются 14 проекций в четырёх семействах — цилиндрическое, коническое,
азимутальное перспективное и азимутальное математическое. Реализованы нормальные,
поперечные и косые аспекты (поворот задаётся `lambda0`/`phiOrigin`/`gamma`),
зоны UTM, спутниковая (вертикальная и наклонная) перспектива, а также
солнечно-синхронная орбита (SOM) для наклонного Меркатора.
...
| `src/components/GlobeScene.tsx` | 3D-сцена, компонует `Globe` / `AuxSurface` / `IntersectionDisks` / `LightSource` / `Rays` и методы визуализации (`VizMethodRenderer`, `Satellite`, `CutLine`, `TouchPointPin`, `CircularSlider`, `Toggle`) |
...
| `azimuthalPerspective` | Stereographic           | —                         | —                                 |
| `azimuthalMath`      | —                         | AzimuthalEqualArea        | AzimuthalEquidistant              |

Вращение использует **отрицательные** знаки: `.rotate([-(lambda0), -phiOrigin, rotZ])`.
`rotZ = -gamma` для конического и азимутального семейств (их наклон действительно
меняет ориентацию в пространстве), но `rotZ = 0` для **цилиндрического**. 2D-карта —
...
> **Модель Земли:** только **сферическая**. Равноугольная азимутальная перспективная
> проекция — это стереографическая; источник света («Источник света») для азимутальных
> перспективных: центр → гномоническая (`geoGnomonic`), антипод → стереографическая
> (`geoStereographic`), бесконечность → ортографическая (`geoOrthographic`),
> «математическая» → аналитическая (равновеликая/равнопромежуточная). Семейство
> `azimuthalMath` использует аналитическое отображение без лучей. Для цилиндрических
> и конических семейств отдельного «источника света» нет — для конуса это вершина
> конуса, для цилиндра глобус разворачивается на цилиндр.
