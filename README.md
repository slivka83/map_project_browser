...
`rotZ = -gamma` для конического и азимутального семейств (их наклон действительно меняет то,
что проецируется), но `rotZ = 0` для **цилиндрического**. 2D-карта — это проекция глобуса НА
развёртываемую поверхность в СОБСТВЕННОЙ системе поверхности; для цилиндра это развёрнутая
труба, которая **ничего не знает про наклон цилиндра в пространстве**. Поэтому `gamma`
намеренно НЕ входит в цилиндрическую проекцию: наклон 3D-трубы лишь поворачивает её в
пространстве, а карта (её плоская развёртка) остаётся неизменной; `lambda0` / `phiOrigin`
по-прежнему задают центральный меридиан / широту. `makeCylindricalProjection(distortion,
scaleFactor)` сохраняет выбранное свойство своим законом высоты со стандартной параллелью на
контакте ±φ_s (φ_s = arccos(`scaleFactor`)): равноугольная → секущая Меркатора, равновеликая →
y ∝ sin φ / cos φ_s, равнопромежуточная → y ∝ φ. Широты за ±`CLIP_LAT` (от оси цилиндра)
зажимаются внутри закона высоты, поэтому труба **конечна** — без ухода в бесконечность у
полюса, как и в 3D. Проекция строится с `precision(0)` (без адаптивной передискретизации),
иначе d3 изгибает прямые параллели в дуги и карта выглядит «яйцом». Конические и азимутальные
проекции зашивают `gamma`. Для конических проекций берётся пара параллелей `parallels([φ1, φ2])`
