...
Сам закон высоты зажимается лишь до ±`CLAMP_LAT` (чуть дальше края) — только чтобы у
Меркатора не уйти в бесконечность на полюсе; всё между `CLIP_LAT` и `CLAMP_LAT` всё равно
отрезается по краю трубы. В экваториальном виде (φ₁ = 0) параллели строятся с `precision(0)`
(без адаптивной передискретизации), иначе d3 изгибает прямые параллели в дуги и карта
выглядит «яйцом»; в наклонном виде (φ₁ ≠ 0) включается адаптивная передискретизация, чтобы
гребни параллелей у края трубы изгибались корректно. Конические и азимутальные
