...
точным зондом закона высоты, без линейной аппроксимации). Оба слоя делят один аналитический
фит по полосе ±`CLIP_LAT` (размер окна постоянен при любом положении ползунков), и каждый
слой обрезан `clipExtent` по СВОЕЙ текущей полосе, пересечённой с экраном; обрезка ставится
строго после финального `translate` (d3 clipExtent работает в экранных пикселях, но обязан
применяться последним — иначе рассинхронизируется и выпускает полярный мусор). Закон высоты
