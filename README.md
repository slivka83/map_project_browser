...
Вращение использует **отрицательные** знаки. Наклон цилиндра геометрически равносилен
наклону **глобуса** относительно неподвижного прямого цилиндра — то есть сдвигает
центральную широту цилиндра, поэтому для цилиндрического семейства `gamma` зашивается в
`phiOrigin`: `.rotate([-lambda0, -(phiOrigin + gamma), 0])` (НЕ как третий компонент roll,
иначе карта скручивается в «яйцо»). Карта **перерисовывается** при наклоне — материки и сетка
смещаются, но сама рамка остаётся прямоугольником с прямыми параллелями (без «яйца»), потому
что проекция строится с `precision(0)`. `makeCylindricalProjection(distortion, scaleFactor)`
...
Конические и азимутальные проекции зашивают `gamma` как roll (их ориентация в пространстве
действительно меняется). Для конических проекций берётся пара параллелей `parallels([φ1, φ2])`
