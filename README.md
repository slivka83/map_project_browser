...
(третий компонент `gamma` — наклон/третье вращение, даёт поперечные/косые аспекты); **для
цилиндрического семейства `gamma` не зашивается в 2D-проекцию** (третий компонент поворота
равен 0) — 3D-цилиндр это жёсткая труба, геометрия которой берётся из ненаклонённой
проекции и лишь поворачивается в пространстве, поэтому развёрнутая 2D-карта должна
совпадать с этой же трубой (остаётся обычной цилиндрической картой, сдвинутой только на
`lambda0`/`phiOrigin`, без сильных искажений). Конические и азимутальные проекции по-прежнему
зашивают `gamma`. Для конических проекций берётся пара параллелей `parallels([φ1, φ2])`
